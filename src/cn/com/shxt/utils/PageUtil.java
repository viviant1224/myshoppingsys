...
public class PageUtil {
	...
	public PageBean queryByPage(String tableName,int nowPage,int iRows,String state){
		...
		pageBean.setRows(rows);
		 String sqlString=" select a.* from (  select rownum rowno,t.* from "+tableName+" t where STATE='"+state+"' and rownum<="+iRows*nowPage+"  order by ID desc  )a  where a.rowno >="+((nowPage-1)*iRows+1);
