	   ...
	   //创建js数组
	     ...
	     if(flag){
	    	 ...
	    	 if(confirm("您确定要删除所选记录?")){
			     ...
			     window.location.href="/myshoppingsys/goodsServlet?flag=delBatch&goodsIdsArray="+goodsIdsArray.join(",");
	    	 }else{
	    		 return;
	    	 }
   ...
   <%
     ...
     %>
   		 ...
   		 <table border="1" align="center" bordercolor="#0099FF"  border="1" id="table" cellspacing="0px">
   		 	...
   		 	<tr>
   		 	 <td> <input type="checkbox" name="checkAll" id="checkAll"  value=""  onclick="checkAllDemo()" /> 全选&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" name="" value="删除" onclick="delByBatch()"></td>
   		 		<td>商品名</td>
   		 		<td>类别</td>
   		 		<td>售价(元)</td>
   		 		<td>出厂日期</td>
   		...
   		//System.out.println("-------------"+list.size());
   		 	...
   		 	%>
   		 		...
   		 		<td><a href="javascript:void(0)" onclick="goodsUpdate(<%=map.get("ID").toString()%>)">修改</a></td>
   		 		<td><a href="javascript:void(0)" onclick="showMore(<%=map.get("ID").toString()%>)">查看更多</a></td>
   		 		<td><a href="../myshoppingsys/goodsServlet?goodsId=<%=map.get("ID").toString()%>&flag=shangjia" target="iframe"><%="上架" %></a></td>
